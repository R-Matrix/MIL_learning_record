...
lemma mul_left_cancel₃ {G : Type} [Group₃ G] {a b c : G} (h : a * b = a * c) : b = c := by
  rw[←one_mul b, ←Group₃.inv_mul, mul_assoc₃, h, ←mul_assoc₃, Group₃.inv_mul, one_mul]
...
{ add_comm := by
    ...
    have : a + (a + b + b) = a + (b + a + b) := by
      calc
      _= (a + a) + (b + b) := by simp only[add_assoc₃]
      _= (1 + 1) * a + (1 + 1) * b := by simp only [one_mul, Ring₃.right_distrib]
      _= (1 + 1) * (a + b) := by simp only [Ring₃.left_distrib]
      _= (a + b) + (a + b) := by simp only [Ring₃.right_distrib, one_mul]
      _= a + (b + a + b) := by simp only [add_assoc₃]
...
class Preorder₁ (α : Type) extends LE₁ α where
  -- 预序关系 : 满足**自反性**和**传递性**  -- `PreOrder`
  le_refl : ∀ a : α , a ≤₁ a
  le_trans : ∀ a b c : α , a ≤₁ b → b ≤₁ c → a ≤₁ c

class PartialOrder₁ (α : Type) extends Preorder₁ α where
  -- 偏序关系 : 满足 自反性 传递性 **反对称性** -- `PartialOrder`
...
instance : OrderedCommMonoid₁ ℕ where
  ...
  mul_le_mul_left _ _ c h:= Nat.mul_le_mul_left c h
...
---- Exercise :
/-  作为练习，您可以回到上面构建的序关系层次结构，
    并尝试引入一个携带“小于”符号 <₁ 的类型类 LT₁，并确保每个预序都带有一个 <₁，其默认值由 ≤₁ 构建，
    并且有一个 Prop 值字段，断言这两个比较运算符之间的自然关系。-/
...
class Preorder₂ (α : Type) extends LT₁ α, LE₁ α where
  le_refl : ∀ a : α , a ≤₁ a
  le_trans : ∀ a b c : α , a ≤₁ b → b ≤₁ c → a ≤₁ c
  lt a b := a ≤₁ b → ¬(b ≤₁ a)
  lt_iff_le_of_not_le : ∀ a b : α , a <₁ b ↔ a ≤₁ b ∧ ¬ (b ≤₁ a)
