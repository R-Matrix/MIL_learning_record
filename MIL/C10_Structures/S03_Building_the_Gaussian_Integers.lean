...
theorem sq_add_sq_eq_zero {α : Type*} [Ring α] [LinearOrder α] [IsStrictOrderedRing α]
  ...
  · intro hez
    have : x ^ 2 = 0 := by
      apply le_antisymm
      · rw[add_eq_zero_iff_eq_neg, hez, neg_le, neg_zero] at *
        apply pow_two_nonneg
      · apply pow_two_nonneg
    rw[this, zero_add] at hez
    constructor <;> apply sq_eq_zero_iff.mp (by assumption)
  · rintro ⟨xeqz, yeqz⟩
    rw[xeqz, yeqz, pow_two, zero_mul, zero_add]
...
theorem norm_eq_zero (x : GaussInt) : norm x = 0 ↔ x = 0 := by
  rw[norm, sq_add_sq_eq_zero, GaussInt.ext_iff, zero_re, zero_im]
theorem norm_pos (x : GaussInt) : 0 < norm x ↔ x ≠ 0 := by
  rw[lt_iff_le_and_ne, Ne, eq_comm, norm_eq_zero, Ne]
