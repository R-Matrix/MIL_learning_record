...
def weightedAverage (lambda : Real) (lambda_nonneg : 0 ≤ lambda) (lambda_le : lambda ≤ 1)
  ...
  x_nonneg := by apply add_nonneg (by apply mul_nonneg lambda_nonneg a.x_nonneg) (by apply mul_nonneg (by linarith) b.x_nonneg)
  y_nonneg := by apply add_nonneg (by apply mul_nonneg lambda_nonneg a.y_nonneg) (by apply mul_nonneg (by linarith) b.y_nonneg)
  z_nonneg := by apply add_nonneg (by apply mul_nonneg lambda_nonneg a.z_nonneg) (by apply mul_nonneg (by linarith) b.z_nonneg)
  sum_eq := by
    calc
      _= lambda * (a.x + a.y + a.z) + (1 - lambda) * (b.x + b.y + b.z):= by ring
      _= lambda + (1 - lambda) := by rw[a.sum_eq, b.sum_eq, mul_one, mul_one]
      _= 1 := by ring
...
def weightedAverage (n : ℕ) (lambda : ℝ) (lambda_nonneg : 0 ≤ lambda) (lambda_le : lambda ≤ 1)
  ...
  NonNeg := by
    intro i
    apply add_nonneg (by apply mul_nonneg lambda_nonneg (a.NonNeg i)) (by apply mul_nonneg (by linarith) (b.NonNeg i))
  sum_eq_one := by
    rw[Finset.sum_add_distrib, ←Finset.mul_sum, ←Finset.mul_sum, sum_eq_one a, sum_eq_one b]
    linarith
