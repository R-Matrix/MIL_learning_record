...
theorem reverse_reverse (as : List α) : reverse (reverse as) = as := by
  induction' as with a as ih
  · rfl
  · rw[reverse, reverse_append, ih, reverse, reverse, nil_append]; rfl

theorem cons_append (as bs : List α) (a : α) : a :: as ++ bs = a :: (as ++ bs) := by
  rfl

theorem append_assoc (as bs cs : List α) : as ++ bs ++ cs = as ++ (bs ++ cs) := by
  induction' as with a as ih
  · rfl
  · rw[cons_append as (bs ++ cs), ←ih]; rfl
...
theorem depth_le_size : ∀ t : BinTree, depth t ≤ size t := by
  intro t
  induction' t with l r h1 h2
  · rfl
  · rw[depth, size]
    calc
    _≤ l.depth + r.depth + 1 := by gcongr; apply max_le_add_of_nonneg (Nat.zero_le _) (Nat.zero_le _)
    _≤ l.size + r.size + 1 := by apply add_le_add (add_le_add h1 h2) (le_refl _)

def flip : BinTree → BinTree
  | empty => empty
  | node l r => r.flip.node l.flip
...
theorem size_flip : ∀ t, size (flip t) = size t
  | BinTree.empty => rfl
  | node l r => by rw[flip, size, size, size_flip l, size_flip r, add_comm r.size];
end BinTree
...
theorem subst_eq_of_not_mem_vars :
    ...
    | var n, x, C, h => by rw[subst, if_neg]; rwa[vars,Finset.mem_singleton, eq_comm] at h;
    | fls ,_ ,_ ,_ => rfl
    | conj A B, n, C, h => by rw[subst,vars,Finset.mem_union,not_or,subst_eq_of_not_mem_vars , subst_eq_of_not_mem_vars] at *; exact h.2; exact h.1
    | disj A B, n , C, h => by rw[subst, vars, Finset.mem_union, not_or, subst_eq_of_not_mem_vars, subst_eq_of_not_mem_vars] at *; exact h.2; exact h.1
    | impl A B, n , C, h => by rw[subst, vars, Finset.mem_union, not_or, subst_eq_of_not_mem_vars, subst_eq_of_not_mem_vars] at *; exact h.2; exact h.1
...
theorem subst_eval_eq : ∀ (A : PropForm) (n : ℕ) (C : PropForm) (v : ℕ → Bool),
  ...
  intro A n C v
  match A with
  | var x => rw[subst,eval]; by_cases h : x = n; rw[if_pos h, if_pos h]; rw[if_neg h, if_neg h, eval]
  | fls => rw[subst, eval, eval]
  | conj A B => rw[subst, eval, eval, subst_eval_eq A, subst_eval_eq B]
  | disj A B => rw[subst, eval, eval, subst_eval_eq A, subst_eval_eq B]
  | impl A B => rw[subst, eval, eval, subst_eval_eq A, subst_eval_eq B]
