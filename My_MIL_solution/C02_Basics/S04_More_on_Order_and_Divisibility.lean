import MIL.Common
import Mathlib.Data.Real.Basic

namespace C02S04

section
variable (a b c d : ℝ)

#check (min_le_left a b : min a b ≤ a)
#check (min_le_right a b : min a b ≤ b)
#check (le_min : c ≤ a → c ≤ b → c ≤ min a b)

example : min a b = min b a := by
  apply le_antisymm
  · show min a b ≤ min b a
    apply le_min
    · apply min_le_right
    apply min_le_left
  · show min b a ≤ min a b
    apply le_min
    · apply min_le_right
    apply min_le_left

example : min a b = min b a := by
  have h : ∀ x y : ℝ, min x y ≤ min y x := by
    intro x y
    apply le_min
    apply min_le_right
    apply min_le_left
  apply le_antisymm
  apply h
  apply h

example : min a b = min b a := by
  apply le_antisymm
  repeat
    apply le_min
    apply min_le_right
    apply min_le_left

example : max a b = max b a := by
  apply le_antisymm
  · show max a b ≤ max b a
    apply max_le
    · apply le_max_right b a
    apply le_max_left b a
  · show max b a ≤ max a b
    apply max_le
    · apply le_max_right a b
    apply le_max_left a b

example : min (min a b) c = min a (min b c) := by
  apply le_antisymm
  · apply le_min
    ·have p: min (min a b) c ≤ min a b := by apply min_le_left
     apply le_trans p
     apply min_le_left
    ·apply le_min
     · apply le_trans (min_le_left (min a b) c) (min_le_right a b)
     · apply min_le_right
  · apply le_min
    · apply le_min
      · apply min_le_left
      · apply le_trans (min_le_right _ _) (min_le_left _ _)
    · apply le_trans (min_le_right  _ _) (min_le_right _ _)

#check sub_le_sub_right
#check min_le_left


theorem aux : min a b + c ≤ min (a + c) (b + c) := by
  apply le_min
  ·apply add_le_add_right
   apply min_le_left
  ·apply add_le_add_right
   apply min_le_right

#check le_of_add_le_add_right

example : min a b + c = min (a + c) (b + c) := by
  apply le_antisymm
  · apply aux
  · have h : min (a + c) (b + c) + -c ≤ min (a+c+ -c) (b+c +-c) +c + -c:= by
     rw [add_assoc,add_neg_cancel,add_zero]
     apply aux (a+c) (b+c)
    rw[add_assoc a, add_assoc b, add_neg_cancel,add_zero,add_zero] at h
    apply le_of_add_le_add_right h

#check abs
#check sub_add_comm
#check (abs_add : ∀ a b : ℝ, |a + b| ≤ |a| + |b|)

/-
***************************************************************
example : |a| - |b| ≤ |a - b| := by
  have h :|a| - |b| + |b|≤ |a - b| + |b| := by
    rw[sub_add_comm,sub_self,zero_add]
    nth_rw 1 [←add_zero a,← neg_add_cancel b,← add_assoc]
    apply abs_add
  apply le_of_add_le_add_right h
***************************************************************
-/

example : |a| - |b| ≤ |a - b| :=
  abs_sub_abs_le_abs_sub a b

end

section
variable (w x y z : ℕ)

example (h₀ : x ∣ y) (h₁ : y ∣ z) : x ∣ z :=
  dvd_trans h₀ h₁

example : x ∣ y * x * z := by
  apply dvd_mul_of_dvd_left
  apply dvd_mul_left

example : x ∣ x ^ 2 := by
  apply dvd_mul_left

example (h : x ∣ w) : x ∣ y * (x * z) + x ^ 2 + w ^ 2 := by
  apply dvd_add
  · apply dvd_add
    · rw[mul_comm x,← mul_assoc]
      apply dvd_mul_left
    apply dvd_mul_left
  · rw[pow_two]
    apply dvd_mul_of_dvd_left h

end

section
variable (m n : ℕ)

#check Nat.gcd

#check (Nat.gcd_zero_right n : Nat.gcd n 0 = n)
#check (Nat.gcd_zero_left n : Nat.gcd 0 n = n)
#check (Nat.lcm_zero_right n : Nat.lcm n 0 = 0)
#check (Nat.lcm_zero_left n : Nat.lcm 0 n = 0)

example : Nat.gcd m n = Nat.gcd n m := by
  apply Nat.dvd_antisymm
  · apply Nat.dvd_gcd
    apply Nat.gcd_dvd_right
    apply Nat.gcd_dvd_left
  · apply Nat.dvd_gcd
    apply Nat.gcd_dvd_right
    apply Nat.gcd_dvd_left

end
