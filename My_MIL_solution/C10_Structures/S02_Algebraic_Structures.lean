...
def addGroupPoint : AddGroup₁ Point where
  ...
  zero := Point.zero
  neg := Point.neg
  add_comm x y:= by rw[Point.add, Point.add]; ext <;> dsimp; repeat rw[add_comm]
  add_assoc x y z := by rw[Point.add, Point.add, Point.add, Point.add]; ext <;> dsimp ; repeat apply add_assoc
  add_zero x := by rw[add]; ext <;> dsimp; repeat apply add_zero
  neg_add_cancel x := by  rw[add, neg, zero]; ext <;> dsimp; repeat apply neg_add_cancel
...
instance hasOneAddGroup₂ {α : Type*} [AddGroup₂ α] : Zero α :=
  ⟨AddGroup₂.zero⟩

instance hasNegAddGroup₂ {α : Type*} [AddGroup₂ α] : Neg α :=
  ⟨AddGroup₂.neg⟩

instance : AddGroup₂ (Point : Type) where
  add := Point.add
  zero := ⟨0,0,0⟩
  neg := Point.neg
  add_assoc x y z := by rw[Point.add];ext <;> dsimp; repeat apply add_assoc
  add_comm x y := by rw[Point.add];ext <;> dsimp; repeat apply add_comm
  add_zero x := by rw[Point.add]; ext <;> dsimp; repeat apply add_zero
  add_neg x := by rw[Point.add, Point.neg]; ext <;> dsimp; repeat apply add_neg_cancel
